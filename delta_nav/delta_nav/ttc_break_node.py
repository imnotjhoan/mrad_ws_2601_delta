...
class TTCBreakNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('forward_angle_range', 5.0)     # degrees - angle range in front of robot to consider
    ...
    def _check_ttc_and_publish(self):
        ...
        # If no laser yet → publish brake OFF
        ...
        if self.should_brake != was_braking:
            if self.should_brake:
                ...
                self.get_logger().info(
                    ...
                    # If not braking, we could optionally re-publish the original command or do nothing
                    # Here we choose to do nothing and let the normal command flow continue
                )
